...
- [ ] 0.1 `extend-sep2-nomenclature` is archived and `openspec/specs/public-api/spec.md` contains
      the canonical-name requirements. The checker encodes that table; starting before it is
      final means encoding a moving target.

## 1. The checker

- [ ] 1.1 Add `tools/check_nomenclature.py`, stdlib only (no third-party imports — it must run
...
- [ ] 1.2 Write the module docstring first, and state the coverage boundary in it: the checker
...
- [ ] 1.3 Add the canonical table as a module-level dict mirroring `docs/seps/sep-0002.rst`,
      ...
      `frf_type`→`frf_form`).
- [ ] 1.4 Implement signature auditing with `ast`, without importing the package: walk public
...
- [ ] 1.5 Implement the divergence rule: report a public parameter or attribute whose name is a
...
- [ ] 1.6 Implement the affix rules: a public parameter ending in `_ind` is a violation naming
      the `_idx` spelling; a public parameter named exactly `n` is a violation naming the
      `n_<plural>` convention.
- [ ] 1.7 Implement the single-uppercase rule: a public parameter whose name is a single
...
- [ ] 1.8 Implement the criterion-function exception: `phi`, `phi_X`, `phi_A` are permitted as
...
- [ ] 1.9 Gate: `python tools/check_nomenclature.py --path <clone of sdypy-model>` reports the
      known divergences (`K`, `M`, `EI`, `org`, `conec`, `frequency`, `n`, `nat_freq`) — the
      checker is proven to fail before any sibling is fixed.

## 2. Tests

- [ ] 2.1 Add `tests/test_nomenclature.py`. Layer one: unit tests over synthetic fixtures written
...
- [ ] 2.2 Add the mirror-drift test: every canonical name in the checker's table appears in
...
- [ ] 2.3 Layer two: conformance tests auditing the installed first-level packages, each marked
      `@pytest.mark.pypi_artifacts`, mirroring the parametrisation of `tests/test_public_api.py`.
- [ ] 2.4 Gate: `pytest -m "not pypi_artifacts" tests/test_nomenclature.py` passes in an
...
- [ ] 3.1 Add a `Check documentation conformance` step to `.github/workflows/docs.yml` running
      `python tools/check_docs.py --path .`, placed beside the existing `check_seps.py` step and
      before `build_index.py`.
- [ ] 3.2 Do NOT add `check_public_api.py` or `check_sibling_template.py` to either workflow:
...
- [ ] 3.3 Verify the docs job still succeeds end to end: `check_seps.py`, `check_docs.py`,
      `build_index.py`, then the Sphinx build.

## 4. Correct the inaccurate claims

- [ ] 4.1 `REQUIREMENTS.md` § Canonical sources: stop implying all four checkers run in this
...
- [ ] 4.2 `REQUIREMENTS.md` § Legend: the `tools/check_*.py` bullet says "also run in CI" —
      qualify it to match 4.1.
- [ ] 4.3 `AGENTS.md` § Common commands: change
...
- [ ] 4.4 Add the new `public-api` and `testing-ci` rows to `REQUIREMENTS.md` for the
      requirements this change adds, attributing only what the checker actually decides.

## 5. Verify

- [ ] 5.1 `openspec validate --strict` clean; `openspec archive enforce-sep2-nomenclature`.
- [ ] 5.2 `pytest -m "not pypi_artifacts"` no worse than the pre-change baseline (Qt-binding
      caveat: `pip install PySide6-Essentials`, else `view`/`model` fail unrelatedly).
- [ ] 5.3 Every invocation in `AGENTS.md` § Common commands is executed as written and reaches a
      verdict. This is the acceptance test for section 4 — the previous state failed it.
- [ ] 5.4 Confirm the two deliberately excluded items are still untouched and still recorded as
